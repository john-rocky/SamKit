import Foundation
import CoreML

/// CLIP BPE tokenizer implemented in pure Swift.
///
/// Loads vocabulary and merge rules from a JSON file exported by
/// `convert_yoloworld_to_coreml.py`. Produces token ID sequences
/// compatible with the CLIP text encoder CoreML model.
public final class CLIPTokenizer {

    // MARK: - Properties

    private let encoder: [String: Int]
    private let decoder: [Int: String]
    private let bpeMerges: [(String, String)]
    private let bpeRanks: [String: Int]
    private let bosToken: Int
    private let eosToken: Int
    private let contextLength: Int

    /// Cache tokenization results
    private var cache: [String: [Int]] = [:]

    // MARK: - Initialization

    /// Initialize tokenizer from a vocabulary JSON file
    public init(vocabularyURL: URL) throws {
        let data = try Data(contentsOf: vocabularyURL)
        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw GroundingError.tokenizationFailed("Invalid vocabulary JSON format")
        }

        guard let encoderDict = json["encoder"] as? [String: Int] else {
            throw GroundingError.tokenizationFailed("Missing 'encoder' in vocabulary")
        }

        guard let mergesList = json["merges"] as? [String] else {
            throw GroundingError.tokenizationFailed("Missing 'merges' in vocabulary")
        }

        self.encoder = encoderDict
        self.decoder = Dictionary(uniqueKeysWithValues: encoderDict.map { ($1, $0) })

        // Parse merges: each line is "token_a token_b"
        self.bpeMerges = mergesList.compactMap { line -> (String, String)? in
            let parts = line.split(separator: " ", maxSplits: 1)
            guard parts.count == 2 else { return nil }
            return (String(parts[0]), String(parts[1]))
        }

        self.bpeRanks = Dictionary(uniqueKeysWithValues:
            bpeMerges.enumerated().map { ($0.element.0 + " " + $0.element.1, $0.offset) }
        )

        let bosStr = json["bos_token"] as? String ?? "<|startoftext|>"
        let eosStr = json["eos_token"] as? String ?? "<|endoftext|>"
        self.bosToken = encoderDict[bosStr] ?? 49406
        self.eosToken = encoderDict[eosStr] ?? 49407
        self.contextLength = json["context_length"] as? Int ?? 77
    }

    // MARK: - Public Methods

    /// Tokenize a text string into token IDs
    public func tokenize(_ text: String) -> [Int] {
        if let cached = cache[text] { return cached }

        let cleaned = text.lowercased().trimmingCharacters(in: .whitespacesAndNewlines)
        let words = cleaned.split(separator: " ").map { String($0) }

        var tokens: [Int] = [bosToken]

        for word in words {
            // Byte-encode the word and add end-of-word marker
            let encoded = byteEncode(word + "</w>")
            let bpeTokens = bpe(encoded)
            for token in bpeTokens {
                if let id = encoder[token] {
                    tokens.append(id)
                }
            }
        }

        tokens.append(eosToken)

        // Truncate to context length
        if tokens.count > contextLength {
            tokens = Array(tokens.prefix(contextLength - 1)) + [eosToken]
        }

        // Pad to context length
        while tokens.count < contextLength {
            tokens.append(0)
        }

        cache[text] = tokens
        return tokens
    }

    /// Tokenize and return as MLMultiArray [1, contextLength]
    public func tokenizeToMLArray(_ text: String) throws -> MLMultiArray {
        let tokens = tokenize(text)
        let array = try MLMultiArray(shape: [1, contextLength as NSNumber], dataType: .int32)
        let ptr = array.dataPointer.bindMemory(to: Int32.self, capacity: contextLength)
        for i in 0..<contextLength {
            ptr[i] = Int32(tokens[i])
        }
        return array
    }

    /// Tokenize multiple queries and return as MLMultiArray [1, N, contextLength]
    public func tokenizeMultiple(_ texts: [String]) throws -> MLMultiArray {
        let count = texts.count
        let array = try MLMultiArray(
            shape: [1, count as NSNumber, contextLength as NSNumber],
            dataType: .int32
        )
        let ptr = array.dataPointer.bindMemory(to: Int32.self, capacity: count * contextLength)

        for (i, text) in texts.enumerated() {
            let tokens = tokenize(text)
            for j in 0..<contextLength {
                ptr[i * contextLength + j] = Int32(tokens[j])
            }
        }

        return array
    }

    /// Clear the tokenization cache
    public func clearCache() {
        cache.removeAll()
    }

    // MARK: - BPE Implementation

    /// Byte-encode a string using CLIP's byte-level encoding
    private func byteEncode(_ text: String) -> [String] {
        var result: [String] = []
        for byte in text.utf8 {
            result.append(byteToUnicode(byte))
        }
        return result
    }

    /// Map a byte value to its Unicode representation (CLIP byte encoding)
    private func byteToUnicode(_ byte: UInt8) -> String {
        let b = Int(byte)
        // Printable ASCII and Latin-1 supplement ranges map directly
        if (33...126).contains(b) || (161...172).contains(b) || (174...255).contains(b) {
            return String(Unicode.Scalar(b)!)
        }
        // Other bytes get shifted to avoid control characters
        return String(Unicode.Scalar(256 + b)!)
    }

    /// Apply BPE merges to a sequence of tokens
    private func bpe(_ tokens: [String]) -> [String] {
        if tokens.count <= 1 { return tokens }

        var word = tokens

        while true {
            // Find the pair with the lowest rank
            var bestPair: (Int, String, String)?
            var bestRank = Int.max

            for i in 0..<(word.count - 1) {
                let pair = word[i] + " " + word[i + 1]
                if let rank = bpeRanks[pair], rank < bestRank {
                    bestRank = rank
                    bestPair = (i, word[i], word[i + 1])
                }
            }

            guard let (_, first, second) = bestPair else { break }

            // Merge the best pair
            var newWord: [String] = []
            var i = 0
            while i < word.count {
                if i < word.count - 1 && word[i] == first && word[i + 1] == second {
                    newWord.append(first + second)
                    i += 2
                } else {
                    newWord.append(word[i])
                    i += 1
                }
            }
            word = newWord

            if word.count == 1 { break }
        }

        return word
    }
}
